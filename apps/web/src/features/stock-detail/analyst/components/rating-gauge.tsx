...
function displayColor(display: number) {
  if (display >= 3.5) return 'text-green-600 dark:text-green-400';
  if (display >= 2.5) return 'text-yellow-600 dark:text-yellow-400';
  return 'text-red-600 dark:text-red-400';
}

export function RatingGauge({ score, labelKo, totalAnalysts, distribution }: RatingGaugeProps) {
  const display = 6 - score;
  const max = Math.max(
  ...
  return (
    <div className="rounded-xl bg-bg-muted p-4">
      <div className="flex items-center justify-between">
        ...
        <InfoTooltip text="애널리스트들의 종합 평점입니다. 5.0에 가까울수록 매수 의견이 강하고, 1.0에 가까울수록 매도 의견이 강합니다." />
      </div>

      <div className="mt-3 flex items-center gap-3">
        <div className="flex h-14 w-14 flex-shrink-0 flex-col items-center justify-center rounded-full bg-bg-surface">
          <span className={cn('text-lg font-bold tabular-nums', displayColor(display))}>
            {display.toFixed(1)}
          </span>
          <span className="text-[10px] text-fg-muted">/5.0</span>
        </div>
        <div>
          <span className={cn('text-sm font-semibold', displayColor(display))}>{labelKo}</span>
