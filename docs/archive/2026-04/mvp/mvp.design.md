...
export interface IndicatorSnapshot {
  ...
  movingAverage: { ma5: number; ma20: number; ma60: number };
...
|--------|------|-------------|:----:|:---:|
| GET | `/api/v1/stocks/search?q={query}` | 티커/회사명 검색 자동완성 (top 10) | — | 1 |
...
| **GET** | **`/api/v1/stocks/{ticker}/detail`** | **상단 4개 블록 통합 (FE 단일 호출). Phase 1 scaffold 구현 — news/aiSignal 은 Phase 2 에서 hydrate.** | — | 1* |
...
```json
{
  ...
  "indicators": {
    ...
    "movingAverage": { "ma5": 177.8, "ma20": 176.3, "ma60": 172.1 },
    "tooltipsKo": {
...
| **Twelve Data** | **`/time_series?symbol=&interval=&outputsize=`** | **캔들** (OHLCV) | **8 req/min, 800 req/day (free)** | AlphaVantage `TIME_SERIES_DAILY` |
| ~~Finnhub~~ | ~~`/stock/candle`~~ | ~~캔들~~ | — | **Free plan 403 (유료 $49.99/mo+). 2026-04-14 Twelve Data 로 교체. 상세: `planning/04-data-sources.md` §4.1 hybrid 전략.** |
